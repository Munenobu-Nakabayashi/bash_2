...
###REMINDER=2	# reminder means "amari" using in COBOL language
REMINDER=$1 	# if using the args
...
function devide_top_250_rec() {		

	append_log_records "start to devide top 250 records"

	ORDER_BY_DESC_FILE=${LOGDIR}/${LOGKANGEN_DIR}/${WORKFILE2}_order_by_desc.txt		# already exists. lf kaigyo

	TOP250LFFILE=${LOGDIR}/${LOGKANGEN_DIR}/${WORKFILE2}_lf_top_250_rec.txt.txt			# work file. lf kaigyo
	TOP250WINFILE=${LOGDIR}/${LOGKANGEN_DIR}/${WORKFILE2}_WIN_top_250_rec_${LASTMONTH}.txt		# deliverables file. crlf kaigyo
	cat /dev/null > ${TOP250LFFILE}
	cat /dev/null > ${TOP250WINFILE}

	append_log_records "start to devide top 250 records from the sorted file to new generated file"
	head -n 250 ${ORDER_BY_DESC_FILE} > ${TOP250LFFILE}
	append_log_records "finish to devide top 250 records from the sorted file to new generated file"

        cat ${TOP250LFFILE}  | sed -e  's/$/\r/g' > ${TOP250WINFILE}
	append_log_records "top 250 crlf kaigyo file is generated"

	rm -f ${TOP250LFFILE}
	append_log_records "top 250 lf kaigyo chukan file is deleted"
	
	append_log_records "finish to devide top 250 records"		
...
function save_top_250_rec_file() {

	append_log_records "start to move ${TOP250WINFILE} into TOP250-${LASTMONTH} folder"

	TOP250WINFILE=${LOGDIR}/${LOGKANGEN_DIR}/${WORKFILE2}_WIN_top_250_rec_${LASTMONTH}.txt

	pushd ${LOGDIR}/${LOGKANGEN_DIR}
	mv ${TOP250WINFILE} ./TOP250-${LASTMONTH}	# we don't have to zip this file. because this file is very small
	popd

	append_log_records "finish to move ${TOP250WINFILE} into TOP250-${LASTMONTH} folder"
...
function execute_or_not(){

	if [ -z ${REMINDER} ]; then
		echo "space is invalid argument"		# check args length
		exit
		terminate_this_process
	elif [ ${REMINDER} -lt 0 ]; then			# args < 0 is invalid
		echo "${REMINDER} is invalid argument"
		exit
		terminate_this_process
	elif [ ${REMINDER} -gt 3 ]; then			# args > 3 is invalid
		echo "${REMINDER} is invalid argument"
		exit
		terminate_this_process
	fi

	if [ $(expr $(date +\%U) \% 4) -eq ${REMINDER} ]; then	# "date +%U" takes the "Week Number" (00 ~ 53 in a year) 
		continue
	else
		exit	# Unconditionally, stop this process
		terminate_this_process
	fi	
...
function main() {
	...
	execute_or_not		# add new 2024.12.17 

	generate_log_file	# update 2024.12.16 
	append_log_records "this procedure is started"
	...
	devide_top_250_rec	# add new 2024.12.18 
	change_lf_for_crlf	# add new 2024.12.09
	save_result_file
	save_top_250_rec_file	# add new 2024.12.18 

        ###### archive_and_remove_files	# the zombie function 
	append_log_records "this procedure is finished"
	save_log_file		# add new 2024.12.09
 
	exit
	
	terminate_this_process	# if this process does not terminate, run this function 
